...
# Test
url = ["http://www.toysrus.com/search/index.jsp?s=A-UnitRank&kw=2016og9&kwCatId=&keywords=2016og9&sr=1&origkw=2016og9&ppg=96&page=1"]
result = []
url.each do |u|
  # loop through url array
  doc = Nokogiri::HTML(open(u))
  doc.css(".prodloop_float").each do |product|
      # textWrapper class will contain title and price info of product
      title = product.css(".prodtitle").text#.delete("\n").gsub("NEW!\u00A0","").gsub(", Only at Macy's","")
      ...
      price = product.css(".ourPrice2").text.scan(/\$(\d+\.\d\d)/).join.to_f
